import type { ITopicsService, IQuestionsService, ImportResult } from "../interfaces";
...
export class ImportService {
        ...
        try {
            ...
            for (const topicData of parsedTopics) {
                try {
                    ...
                    result.errors.push(`Failed to import topic '${topicData.name}': ${msg}`);
                }
            }
    ...
    private parseMarkdown(content: string): ParsedTopic[] {
        ...
        const sections = normalized.split(/^##\s+/m).filter(s => s.trim().length > 0);
        ...
        for (const section of sections) {
            ...
            // Split questions by "### "
            const questionSections = body.split(/^###\s+/m).filter(q => q.trim().length > 0);
