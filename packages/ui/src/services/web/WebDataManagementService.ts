...
import type { Topic, Question, QuestionProgress, QuizSession } from "@code-notes/shared";
...
export class WebDataManagementService implements IDataManagementService {
    ...
    async exportDatabase(filename: string = "code-notes-backup.json"): Promise<ExportResult> {
        try {
    ...
    async importDatabase(content: string, merge: boolean): Promise<GenericImportResult> {
        try {
            ...
            if (!merge) {
                // Clear all tables
                await db.transaction('rw', db.topics, db.questions, db.progress, db.quizSessions, async () => {
            ...
            await db.transaction('rw', db.topics, db.questions, db.progress, db.quizSessions, async () => {
                if (merge) {
                    ...
                    const existingTopicIds = new Set(await db.topics.toCollection().primaryKeys());
                    const newTopics = topics.filter(t => !existingTopicIds.has(t.id));
                    if (newTopics.length > 0) await db.topics.bulkAdd(newTopics);

                    const existingQuestionIds = new Set(await db.questions.toCollection().primaryKeys());
                    ...
                    const existingProgressIds = new Set(await db.progress.toCollection().primaryKeys()); // [questionId]
                    // Progress primary key is questionId? Yes.
                    const newProgress = progress.filter(p => !existingProgressIds.has(p.questionId));
                    if (newProgress.length > 0) await db.progress.bulkAdd(newProgress);

                    const existingSessionIds = new Set(await db.quizSessions.toCollection().primaryKeys());
                    const newSessions = sessions.filter(s => !existingSessionIds.has(s.id));
                    if (newSessions.length > 0) await db.quizSessions.bulkAdd(newSessions);
